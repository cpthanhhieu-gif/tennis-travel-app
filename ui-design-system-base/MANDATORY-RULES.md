...
|---|---|
| `ui-design-system-base/BRAND-IDENTITY-INPUT.md` | Brand identity Vietravel — màu, font, tone, cá tính thương hiệu |
| `tennis-travel-app/app/globals.css` | Token đã khai báo (`@theme`) — chỉ dùng class từ đây, không hardcode hex/px |
| `ui-design-system-base/tokens/` | Giá trị nền tảng: màu, font, spacing, effects, breakpoints |
| `ui-design-system-base/components/` | Spec từng component — đọc trước khi tạo mới |
| `ui-design-system-base/rules/` | Accessibility, layout, interaction, naming rules |
...
```
✅ ĐÚNG: text-[#0046C1], bg-white, border-[#DDDDDD], text-[#000E1A]
❌ SAI:  color: '#0046C1', backgroundColor: '#fff', border: '1px solid #ddd'
```

Không được tự ý thêm màu mới. Nếu cần màu mới → hỏi user, cập nhật `globals.css` và `ui-design-system-base/tokens/colors.md`.
...
## 12. Z-INDEX — THEO LAYER QUY ĐỊNH

| Dùng cho | Class |
|---|---|
| Nội dung thường | `z-0` |
| Card hover | `z-10` |
| Dropdown, popover, select | `z-[100]` |
| Top header, bottom nav | `z-[200]` |
| Modal, dialog, bottom sheet | `z-[300]` |
| Toast, XP notification | `z-[400]` |
| Tooltip | `z-[500]` |

Không được dùng z-index tùy tiện ngoài bảng trên.

---

## 13. ICONS — CHỈ DÙNG LUCIDE REACT
...
## 14. CHUẨN HÓA TÊN CLASS — BẮT BUỘC

Tailwind v4 tự map `--color-*` và `--shadow-*` trong `@theme` thành utility class. **Không được dùng dạng `[var(--...)]` hay `[#hex]` khi đã có token name.**

| ❌ Sai | ✅ Đúng |
|---|---|
| `bg-[var(--color-brand-primary)]` | `bg-brand-primary` |
| `text-[var(--color-neutral-90)]` | `text-neutral-90` |
| `shadow-[var(--shadow-card)]` | `shadow-card` |
| `text-[#0046C1]` | `text-brand-primary` |
| `bg-[#000E1A]` | `bg-neutral-90` |
| `border-[#DDDDDD]` | `border-neutral-10` |

**Ngoại lệ hợp lệ** — giữ `[var(--...)]` khi token nằm trong `:root` (không phải `@theme`):
- `h-[var(--tour-card-image-height)]` ✅
- `h-[var(--destination-card-image-height)]` ✅
- `style={{ borderRadius: "var(--radius-bottom-sheet)" }}` ✅ (multi-value không map được thành class)

---

## CHECKLIST TRƯỚC KHI SUBMIT CODE

- [ ] Đã đọc `BRAND-IDENTITY-INPUT.md` và các file trong `ui-design-system-base/`
